  ...
  return (
    <header className='flex items-center justify-between px-4'>
        <div className="container flex items-center justify-between py-7">
            <h1 className='text-3xl font-bold text-gray-900 dark:text-gray-200'>MB</h1>
            <nav className='flex items-center gap-5 md:gap-10'>
                <ul className='flex items-center gap-5 text-gray-700 dark:text-gray-200'>
                    {links.map((item)=><li key={item.link} className='text-xs md:text-base'><Link href={item.link}>{item.label}</Link></li>)}
                </ul>
                <ThemeSwitcher/>
            </nav>
        </div>
